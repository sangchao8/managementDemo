<template>
    <div class="">
        ...
        <!--<div id="label"></div>-->
        <router-link to="/threeJs" class="home">Go 首页</router-link>
    </div>
</template>
...
<script>
    ...
    /*import { inflate } from "three/examples/js/libs/inflate.min.js";*/
    ...
    export default {
        ...
        methods: {
            ...
            initRenderer () {
                ...
                this.renderer.setClearColor('rgb(0,0,0)', 1);//设置渲染的背景颜色
            ...
            initAgvCar(){
                ...
                MTLLoader1.load(`/models/q.mtl`, function(materials) {
                    // 返回一个包含材质的对象MaterialCreator
                    console.log(materials); //obj的模型会和MaterialCreator包含的材质对应起来
                    OBJLoader1.setMaterials(materials);
                    OBJLoader1.load(`/models/q.obj`, function(obj) {
                        console.log(obj);
                        let mesh = obj;
                        mesh.translateY(0);
                        mesh.translateX(-1380);
                        mesh.translateZ(0);
                      /*  mesh.position.x= 800,
            ...
            initfbx () {
                ...
                loader.load("/models/qt.fbx", function(obj) {
                    console.log(obj);//查看加载后返回的模型对象
                    let mesh = obj;
                    mesh.translateY(0);
                    mesh.translateX(-1380);
                    mesh.translateZ(-200);
                /*    mesh.scale.set(10,10, 10); //放大obj组对象*/
                    that.addObject(mesh);//返回的组对象插入场景中
                })
            },
            ...
            initBuilding () {
                ...
                let airSwitchObjects = {
                    objects: [
                        //空开
                        //第一组空开
                        {
                            objectName: '空开1#开',
                            objectType: "airSwitch",
                            property:'airSwitch',
                            statue:'open',//必有，灯的 状态
                            address:'办公室1灯',//地理位置必有，关联控制它的灯
                            length: 80,
                            width: 15,
                            height: 80,
                            position: {
                ...
                airSwitchObjects.objects.forEach(function (item,index){
                    let airswitch =  that.floor(item);
                    that.addObject(airswitch);
                })
        ...
        return group;
            ...
            initLight (){
                ...
                //点光源
                /*that.pointLight1 = new THREE.PointLight('rgb(255,56,83)', 3, 150);
                that.pointLight1.position.set( -1540,110,360 );
                that.pointLight1.castShadow = true;
                    /!*  pointLight1.target = target;*!/
                    this.addObject(that.pointLight1);
                that.pointLight1.visible = true;//控制灯的显示隐藏
                    this.addObject(new THREE.PointLightHelper(that.pointLight1, 20, 'rgb(255,155,31)'));*/
                    ...
                    objects: [
                        {
                            objectName: '办公室1灯',
                            objectType: "light",
                            property:'pointLight1',//灯光标识
                            statue:true,
                            area:300,//光照区域
                            size:20,//灯体大小
                            airswitch:'空开1',
                            color:'rgb(255,155,31)',//灯光色和灯色
                            position: {
                                x: -1440,
                                y: 110,
                                z: -300
                ...
                lightObject.objects.forEach(function (item,index) {
                    if(item.objectType == 'light'){
                        let pointLight =  that.getPointLight(item,that);
                        that.addObject(pointLight);
                        that.addObject(new THREE.PointLightHelper(pointLight, item.size, item.color));
                        that.pointLight.push(pointLight)
                    }
                })
            },

            getPointLight(P,that){
                let pointLight = new THREE.PointLight(P.color, 3, P.area);
                pointLight.position.set( P.position.x,P.position.y,P.position.z );
                pointLight.castShadow = true;
                /*  pointLight1.target = target;*/
             /*   that.addObject(pointLight);*/
                pointLight.visible = P.statue;//控制灯的显示隐藏
                pointLight.address = P.objectName;//灯的具体位置
               /* that.addObject(new THREE.PointLightHelper(pointLight, P.size, P.color));*/
                return pointLight;
            },
            ...
            ObjectSelect(_scene, _camera,outlinePass) {
                ...
                function onMouseClick(event) {
                    ...
                    if(intersects.length == 0){
                        ...
                        //点击了空开
                        if(intersects[0].object.objectType == "airSwitch"){
                            //点击需要显示数据3d模块
                           /* if(intersects[0].object.name == ""){
                            ...
                            //控制开关
                            if(intersects[0].object.name=='空开1#开'){
                                that.pointLight.forEach(function (item,i) {
                                    if(item.address==intersects[0].object.address){
                                        that.pointLight[i].visible=true
                                    }
                                })
                            }
                            if(intersects[0].object.name=='空开1#关'){
                                that.pointLight.forEach(function (item,i) {
                                    if(item.address==intersects[0].object.address){
                                        that.pointLight[i].visible=false
                                    }
                                })
                            }
                            //选中物体数据清空
